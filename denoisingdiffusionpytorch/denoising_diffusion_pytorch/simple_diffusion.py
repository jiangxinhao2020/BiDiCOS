...
def logsnr_schedule_cosine(t, logsnr_min = -15, logsnr_max = 15):
    ...
    shift2 = -2 * (log(torch.sin((t_min + t * (t_max - t_min)))))
    return -2 * log(torch.tan(t_min + t * (t_max - t_min)))  + shift2
...
class GaussianDiffusion(nn.Module):
    ...
    def q_sample(self, x_start,y_start, times, noise = None):
        ...
        y_noised = y_start * alpha + noise * sigma
        return x_noised,y_noised, log_snr
