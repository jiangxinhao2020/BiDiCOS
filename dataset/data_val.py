...
class PolypObjDataset(data.Dataset):
    def __init__(self, image_root, gt_root, depth_root,trainsize, mean=None, std=None, randomPeper=True,
    ...
    def __getitem__(self, index):
        ...
        # data augumentation
        # image, gt,depth = self.aug_transform(image=np.asarray(image), mask=np.asarray(gt), depth_mask=np.asarray(depth)).values()
        # image, gt,depth = albumentations.PadIfNeeded(*image_size[::-1], border_mode=0)(image=image, mask=gt, depth=depth).values()
        ...
        # depth = randomPeper(depth) if self.do_randomPeper else Image.fromarray(depth)
    ...
    def filter_files(self):
        assert len(self.images) == len(self.gts) ==len(self.depths) and len(self.gts) == len(self.images) ==len(self.depths)
        images = []
        gts = []
        depths = []
