...
class CondGaussianDiffusion(GaussianDiffusion):
    ...
    def forward(self, img,depth, cond_img, seg=None, extra_cond1=None, extra_cond2=None,*args, **kwargs):
        ...
        return self.p_losses(img, depth,times, cond_img, seg, extra_cond1,extra_cond2, *args, **kwargs)

    def p_losses(self, x_start,y_start, times, cond_img, seg=None, extra_cond1=None,extra_cond2=None, noise=None, *args, **kwargs):
        ...
        gt_pred,depth_pred = self.model(torch.cat([x, extra_cond1], dim=1),torch.cat([y, extra_cond2], dim=1), log_snr, cond_img)
        ...
        if self.loss_type == 'l2':
            ...
            return self.loss_type(gt_pred, target)  + (F.mse_loss(depth_pred, depth) + F.l1_loss(depth_pred, depth)) / 2

    @torch.no_grad()
    def sample(self, cond_img, extra_cond1=None,extra_cond2=None, verbose=True):
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, cond_img, extra_cond1,extra_cond2, verbose=True):
        self.history = []
        img = torch.randn(shape, device=self.device)
        depth = torch.randn(shape, device=self.device)
        ...
        for i in tqdm(range(self.num_sample_steps), desc='sampling loop time step', total=self.num_sample_steps,
            ...
            img,depth = self.p_sample(img,depth, conditioning_features, extra_cond1,extra_cond2, times, times_next)

        img.clamp_(-1., 1.)
        img = unnormalize_to_zero_to_one(img)
        depth.clamp_(-1., 1.)
        depth = unnormalize_to_zero_to_one(depth)
        return img,depth

    @torch.no_grad()
    def p_sample(self, x,y, cond, extra_cond1,extra_cond2, time, time_next):
        batch, *_, device = *x.shape, x.device

        model_mean1,model_mean2, model_variance = self.p_mean_variance(x=x,y=y, cond=cond, extra_cond1=extra_cond1,extra_cond2=extra_cond2, time=time,
                                                          time_next=time_next)
    ...
    def p_sample_g(self, x, y,cond, extra_cond, time, time_next):
        batch, *_, device = *x.shape, x.device

        model_mean, model_variance = self.p_mean_variance(x=x, y=y, cond=cond, extra_cond=extra_cond, time=time,
                                                          time_next=time_next)
    ...
    def p_mean_variance(self, x,y, cond, extra_cond1,extra_cond2, time, time_next):
        ...
        pred,depth = self.model.sample_unet(torch.cat([x, extra_cond1], dim=1),torch.cat([y, extra_cond2], dim=1),
                                      batch_log_snr, cond)
